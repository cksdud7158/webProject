...
<title>FootBallTogether - 가입신청 리스트</title>
...
<script type="text/javascript">
	...
	$(function() {
		$('button[type=submit]').on('click', function(){
			...
			location.href='AllJoinRequest.do';
		});//on		
	}); //ready
...
<body class="bg-theme bg-theme1">
	...
	<div id="wrapper">
		...
		<div class="content-wrapper">
			...
			<div class="card">
		  <div class="card-body">
			<h4 class="card-title" style="font-size:20px">All Teams in FootBallTogether</h4>
            ...
            <table class="table table-hover" style="text-align:center">
                <thead>
                  <tr>
                    ...
                    <th scope="col" style="font-size: 15px;">가장 좋아하는 팀</th>
                    <th scope="col" style="font-size: 15px;">포지션</th>
                    <th scope="col"style="font-size: 15px;">키/몸무게</th>
                    <th scope="col"style="font-size: 15px;">스피드</th>
                    <th scope="col"style="font-size: 15px;">패스</th>
                    <th scope="col"style="font-size: 15px;">드리블</th>
                    <th scope="col"style="font-size: 15px;">수비</th>
                    <th scope="col"style="font-size: 15px;">체격</th>
                    <th scope="col"style="font-size: 15px;"></th>
                  </tr>
                </thead>
                <tbody id="resultView">
                  <c:forEach items="${uVoList}" var="request" varStatus="status">
                  	<tr>
                  	  <th scope="row">
                  	  <img src="${request.photo}" style="width:40px;height:40px;"></th>
                  	  <td>${request.name}</td>
                  	  <td>${request.gender}</td>
                  	  <td>${request.favTeam1}</td>
                  	  <td>${request.pVo.position}</td>
                  	  <td>${request.pVo.height} / ${request.pVo.weight}</td>
                  	  <td>${request.pVo.speed}</td>
                  	  <td>${request.pVo.pass}</td>
                  	  <td>${request.pVo.dribble}</td>
                  	  <td>${request.pVo.defence}</td>
                  	  <td>${request.pVo.physical}</td>
                  	  <td>
                  	  	<button type="submit" class="btn btn-light px-5" id="allowToJoin" value="${request.userId}"><i class="icon-lock"></i> 수락</button>
                  	  	<button type="button" class="btn btn-light px-5" id="rejectToJoin" value="${request.userId}"><i class="icon-lock"></i> 거절</button>
                  	  </td>
                  </c:forEach>
                </tbody>
              </table>
